classdef LLEMap_time_block_cv < handle
    ...
    methods
        ...
        % Constructor
        function [ obj ] = LLEMap_time_block_cv( k, lambda, nfolds )
            % Inputs:
            %   k
        ...
        % Fit mapping
        function [] = fit( obj, X, Y, t, tmargin )
            % Inputs:
            %   X
            ...
            cvp = tspartition(t, obj.nfolds, tmargin);
            ...
            for it_lambda = 1 : nlambda
                ...
                for it_k = 1 : nk
                    ...
                    for it_cv = 1 : obj.nfolds
                        ...
                        % compute test error using lle mapping
        ...
        % Compute error using lle mapping
        function [ err ] = compute_lle_error( obj, X, Y, Xi, nb, lambda, Ytest)
            
            % predict test set outputs
            Yhat = obj.lle_mapping( X, Y, Xi, nb, lambda );
            
            % compute error
            err = sum((Ytest(:) - Yhat(:)).^2);
        end % compute_lle_error()
